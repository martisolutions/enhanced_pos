<template>
	<div class="epos-header bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-4 flex justify-between items-center">
		...
		<div class="epos-header-actions flex gap-2 items-center">
			<button
				class="btn btn-primary bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium transition"
				@click="$emit('openInvoiceForm')"
			...
			<template v-for="plugin in headerPlugins" :key="plugin.name">
				...
				<button
					...
					:disabled="plugin.disabled ? plugin.disabled({ hasOpenSession, user }) : !hasOpenSession"
					@click="plugin.action ? plugin.action({ hasOpenSession, user }) : null"
				>
